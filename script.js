...
// Функція для визначення напрямку свайпу
...
// Слухачі подій для треку каруселі
...
document.addEventListener("submit", function (e) {
  ...
  if (
    e.target.name === "popup-order" ||
    e.target.classList.contains("order-form")
  ) {
    e.preventDefault();
    ...
    fetch(URI_API, {
      ...
      .then((res) => {
        if (res.ok) {
          ...
          alert(
            "Помилка при відправці. Перевірте, чи запущений бот (натисніть START).",
          );
        }
      })
      .catch((err) => {
